...
class _WeatherDaysTabState extends State<WeatherDaysTab> {
          ...
          child: Column(
            children: [
              ...
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    Image.asset(
                      imgByStatus(
                          status: getWeekDay(weekday: DateTime.now().weekday)
                                  ?.manha
                                  ?.tempo ??
                              ""),
                      height: 132,
              ...
              Column(
                  children: daysWeek
                      .sublist(
                          daysWeek.indexOf(daysWeek.firstWhere((element) =>
                                  element?.number == DateTime.now().weekday)) +
                              1,
                          daysWeek.length)
                      .map((dayweek) => WeatherDayComponent(
                            day: dayweek?.name ?? "",
                            status: dayweek?.manha?.tempo ?? "",
                            imgstatus: imgByStatus(
                                status: dayweek?.manha?.tempo ?? ""),
  ...
  DayWeekEntity? getWeekDay({required int weekday}) {
    switch (weekday) {
      case 1:
        return widget.time.segunda;
      case 2:
        return widget.time.terca;
      case 3:
        return widget.time.quarta;
      case 4:
        return widget.time.quinta;
      case 5:
        return widget.time.sexta;
      case 6:
        return widget.time.sabado;
      case 7:
        return widget.time.domingo;
      default:
        return widget.time.segunda;
    }
  }
